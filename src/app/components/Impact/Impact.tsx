  ...
  return (
    <section
      ...
      /* Desktop: h-[520vh] == 1.3 viewports of scroll per stat (STATS has 4).
         Mobile: a much shorter track so each number flips after a small scroll.
         The sticky panel inside holds the number in place while the stats pass. */
      className="relative h-[520vh] w-full bg-background max-md:h-[240vh]"
    >
      ...
      <div className="sticky top-0 flex h-screen flex-col items-center justify-center max-md:[transform:translateZ(0)]">
        ...
        {/* Clickable progress indicator: vertical on the right edge for desktop,
            a horizontal row along the bottom on phones (so it never sits over
            the number). */}
        <div className="absolute right-10 top-1/2 flex -translate-y-1/2 flex-col items-center gap-3 max-md:bottom-10 max-md:right-1/2 max-md:top-auto max-md:translate-x-1/2 max-md:translate-y-0 max-md:flex-row">
          {STATS.map((s, i) => (
            <button
              key={s.title}
              type="button"
