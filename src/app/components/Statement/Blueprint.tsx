// Faint construction "blueprint" line art — the backdrop of the statement, and
// of the scene that climbs over it. The layer is fixed to the viewport rather
// than to a section: it arrives with the statement panel and then stays exactly
// where it is while Solution's frames rise in front of it. That is the whole
// point of it living here — only the picture travels, never the shapes it is
// drawn over.
//
// Each stroke has pathLength=1 so the shared .bp-line rule can draw it in and
// out on a loop (see globals.css). Negative delays desync the strokes so the
// drawing feels continuous rather than starting all at once.
export default function Blueprint() {
  return (
    // Transparent until the statement's timeline fades it up: a fixed layer is
    // on screen from the very first paint, so without this the line art would
    // sit over the hero for the whole way down. The id is how Solution's scene
    // reaches it to dissolve it again at the end (see Solution.tsx).
    <div
      id="statement-blueprint"
      aria-hidden="true"
      className="pointer-events-none fixed inset-0 z-0 opacity-0"
    >
      {/* The mask dissolves the line art over the bottom third of the screen,
          so the strokes never stop dead along the edge of the viewport.
          Unprefixed only — pairing it with -webkit-mask-image makes the
          minifier collapse the two and drop the standard property modern
          browsers actually read. */}
      <svg
        className="h-full w-full opacity-20 [mask-image:linear-gradient(to_bottom,#000_62%,transparent_100%)]"
